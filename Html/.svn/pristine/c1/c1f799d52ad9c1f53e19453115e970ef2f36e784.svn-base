        ...
        <div class="section">
            <div class="layer-child layer-manim " style="">
               <button class="layer-end" v-if="layerOptions.type == 'prop'" @click="onlayerback('confirm')"></button>
                ...
                <div class="layer-fbtn" v-if="layerOptions.type != 'prop'"><span v-if="layerOptions.type == 'confirm'"><div class="btn btncancel" @click="onlayerback('cancel')">{{ cancelBtn }}</div></span><span><div class="btn btnok" @click="onlayerback('confirm')">{{ confirmBtn }}</div></span></div>
            </div>
        </div>
    </div>
</div>
...
export default {
  ...
  methods  : {
      onlayerback: function (msg,shade) {
            //点击蒙版
